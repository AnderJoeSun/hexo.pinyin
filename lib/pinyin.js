...
//  }
  ...
  // 支持正念多蜜博客的标签按拼音排序，详细使用说明：https://zhengnian.me/2015/12/25/source.projects.of.this.blog/
  compare (tagA, tagB) {
    const pinyinA = this.convert(tagA.name, DEFAULT_OPTIONS);
    const pinyinB = this.convert(tagB.name, DEFAULT_OPTIONS);
    return String(pinyinA).localeCompare(String(pinyinB));
  }
